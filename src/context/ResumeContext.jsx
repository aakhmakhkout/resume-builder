...
export function ResumeProvider({ children }) {
  ...
  const renameSkillCategory = (categoryId, name) => {
    // Store exactly what the user typed, including an empty string, so the
    // controlled input can actually be cleared while editing. A fallback name is
    // applied only at render/export time (see normalizeSkillCategory), not here —
    // forcing a fallback on every keystroke made the field snap back to "Skills"
    // the instant it was emptied, so it could never be fully cleared.
